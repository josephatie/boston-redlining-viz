...
export default function CrimeViz() {
    ...
    return (
      <section className="w-screen h-screen flex">
        <aside className="w-1/2 h-screen sticky top-0 flex justify-center items-center p-6" style={{ backgroundColor: 'rgba(0, 0, 0, 0.1)' }}>
            ...
            {neighborhoods.map((nb, idx) => (
              <div key={nb.id} className="w-72 h-80 cursor-pointer group" onClick={() => handleFlip(idx)} style={{ perspective: '1200px' }}>
                <div className="relative w-full h-full" style={{ transformStyle: 'preserve-3d', transform: flipped[idx] ? 'rotateY(180deg)' : 'rotateY(0deg)', transition: 'transform 0.7s ease-in-out' }}>
                  <div className="absolute w-full h-full rounded-2xl shadow-xl overflow-hidden" style={{ backfaceVisibility: 'hidden' }}>
                    ...
                    <div className="p-4 bg-white h-1/4 flex flex-col justify-center">
                      <h3 className="text-lg font-bold">{nb.name}</h3>
                      <p className="text-sm text-gray-700">{nb.stat}</p>
                    </div>
                  </div>
                  <div className="absolute w-full h-full rounded-2xl shadow-xl bg-white p-4 overflow-y-auto" style={{ transform: 'rotateY(180deg)', backfaceVisibility: 'hidden' }}>
                    <h3 className="text-lg font-bold mb-2">{nb.name} Story</h3>
                    <p className="text-sm text-gray-800 leading-relaxed">{nb.story}</p>
                  </div>
                </div>
              </div>
            ))}
        ...
        <main className="w-1/2 h-screen p-8 flex flex-col">
          <h2 className="text-4xl font-bold mb-4">Crime</h2>
